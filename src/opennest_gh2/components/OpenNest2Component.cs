...
namespace opennest_gh2.components
{
    ...
    public class OpenNest2Component : NestComponentBase
    {
        ...
        protected override void AddOutputs(OutputAdder outputs)
        {
            ...
            outputs.AddGeneric("Attributes", "Attributes", "Attribute geometry carried with each part. One attribute input => {part}; multiple Attributes inputs (via +) => {part; input-port} sub-branches.", Access.Tree);
        }
        ...
        protected override void Assemble(IDataAccess access)
        {
            ...
            // Attributes grouped by output path: key = (part index i, port) with port = -1 when only one
            // attribute input exists (=> flat {part}); otherwise port = source input index (=> {part; port}).
