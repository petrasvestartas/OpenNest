...
namespace opennest_2
{
    ...
    public static class NestOptionCatalog
    {
        ...
        public static List<NestOption> Collision() => new List<NestOption>
        {
            ...
            NestOption.Choice("batch", "Batch", new[] { "Off", "On" }, new[] { "0", "1" }, 1, "Keep related parts together for sorting. ON (default): each Geometry data-tree BRANCH is a batch, nested into its own tight block, and placed ONE BATCH PER SHEET (branch 0 -> sheet 0, branch 1 -> sheet 1, ...; more batches than sheets wrap round-robin onto the sheets). A flat (single-branch) input is one combined nest unless 'Batch Size' is set. OFF: ignore branches, nest everything combined onto the fewest sheets (densest)."),
