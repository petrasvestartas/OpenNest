    ...
    public class Background
    {
        ...
        public static SheetPlacement placeParts(NFP[] sheets, NFP[] parts, SvgNestConfig config, int nestindex)
        {
            ...
            while (parts.Length > 0)
            {
                ...
                fitness += 1; // +1 per new sheet opened (canonical; lower fitness is better). Was += sheetarea (millions), which dwarfed the within-sheet packing term so tighter packs were invisible to the GA.
                ...
                for (i = 0; i < parts.Length; i++)
                {
                    ...
                    // NOTE: loop the ROTATION COUNT (config.rotations), not 360/rotations — the latter ran
                    // 90 times at rotations=4, re-trying the same 4 grid angles ~22x for any part that
                    // can't fit (wasted rotatePolygon + NFP lookups).
                ...
                {
                    fitness += minwidth.Value / sheetarea; // canonical: normalized packing width. (minarea = width*2+height stays the LOCAL position-selection metric; adding it to fitness dominated the per-sheet +1.)
                }
            ...
            {
                fitness += 2; // canonical: +2 per unplaced part. Comparable to opening ~2 sheets, so the GA strongly prefers placing everything, without the term dwarfing the packing signal (was 1e8*areafrac).
            }
        ...
        public void BackgroundStart(DataInfo data)
        {
            ...
            for (var i = 0; i < parts.Count; i++)
            {
                ...
                //rotations[i] = GeometryUtil.remap(rotations[i], -10 * 0.5f, 10 * 0.5f);
                //Rhino.RhinoApp.WriteLine(rotations[i].ToString());
                //float rotation_limit = 10.0f;
                //float remapped_value = -rotation_limit + (rotations[i] - 0.0f) * (rotation_limit - (-rotation_limit)) / (360.0f - 0.0f);

                parts[i].rotation = rotations[i];
                //parts[i].rotation = -1;
                //parts[i].rotation = remapped_value;
                parts[i].id = ids[i];
        ...
        public NfpPair process(NfpPair pair)
        {
            ...
            ///////////////////
            var Ac = clipper_util.ScaleUpPaths(A, 10000000);

            var Bc = clipper_util.ScaleUpPaths(B, 10000000);
        ...
        public static NFP[] NewMinkowskiSum(NFP pattern, NFP path, int type, bool useChilds = false, bool takeOnlyBiggestArea = true)
        {
            ...
            var ac = clipper_util.ScaleUpPaths(pattern, 10000000);
            ...
            {
                var bc = clipper_util.ScaleUpPaths(path, 10000000);
                for (var i = 0; i < bc.Length; i++)
                {
                    bc[i].X *= -1;
                    bc[i].Y *= -1;
