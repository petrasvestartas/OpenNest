...
namespace nest_lib
{
    public partial class NestingContext
    {
        ...
        public void NestIterate(int max_iterations)
        {
            // Build parts + offset tree ONCE. Re-running init() every iteration (the old behaviour, with
            // the Iterations==0 guard commented out) repeated the expensive offsetTree on every step and
            // was the dominant slowdown. The GA clones from this template each step, so it is never mutated.
            if (Iterations == 0)
                init();

            //do not run solver to preview only the input data
            if (max_iterations == 0)
                return;


            Nest.launchWorkers(partsLocal.ToArray());
            var plcpr = Nest.nests.FirstOrDefault();


            if (plcpr != null && (current == null || plcpr.fitness < current.fitness))
        ...
        // Run one full GA GENERATION = evaluate populationSize candidates (one per launchWorkers call; the
        // GA evolves itself inside launchWorkers once a population is fully scored). The best-so-far is
        // tracked in `current` and only ever improves (single elitism). The host's "Iterations" input maps
        // to the number of NestGeneration() calls — i.e. Iterations == generations.
        public void NestGeneration()
        {
            if (Iterations == 0)
                init();
        ...
        public void AssignPlacement(SheetPlacement plcpr)
        {
            ...
            this.SheetsNotUsed = emptySheets.Length;
